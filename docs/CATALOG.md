...
The fallback chain for an unset start is: `song.startAtMs` → `song.media.onsetPadMs` → `30` (default LAME padding). Do not skip a deliberate fade or melody merely because it is soft. Set `clueGainDb` from 0–12 dB for those cases; clue playback is limited. A quiet master is a different problem: `playbackGainDb` (also 0–12, derived from measured `bodyDb` toward -16 dBFS) raises both clues and the reveal so whisper-pop sits nearer brickwalled pop. Full-song reveal still keeps the original mix when the body is already loud. Audible static cannot be distinguished reliably from intentional texture by level measurements, so known static receives an explicit skip such as the tracked `No One Noticed` override.
...
`releaseYear` drives the explicit Modern (2020+), 2010s, 2000s, and pre-2000 era filters; songs without a verified year remain available only in `All eras`. Catalogue generation collapses the stored free-form genre tags into broad Pop, Hip-hop/Rap, R&B/Soul, Rock/Alternative, Dance/Electronic, and Other groups. When an older imported row is unclassified, it inherits broad groups from other reviewed songs by the same credited artist; truly unmatched rows remain Other. A track may belong to more than one broad group. Players can select multiple eras and genres: alternatives within either group use OR matching, and the era group is combined with the genre group using AND matching. The selected filter arrays and the no-repeat draw history are browser-local preferences; no account or database is required.
