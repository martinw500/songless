...
The broad intake pool lives in `data/song-longlist.json`, with a readable copy in `data/song-longlist.txt`. Its source snapshot contains 1,125 tracks reported above one billion Spotify streams. After the recorded pruning decisions, 823 active billion-stream rows remain; founder/Gen-Z/current-recognition picks bring the combined active pool to 1,125 unique songs. Of the active tracks, 106 have been explicitly accepted in `data/song-longlist-keeps.json`. Run `npm run audit:coverage` for the current candidate and playable coverage instead of relying on a stale hand-written total.
...
3. Run `npm run init:sources` to create or safely sync ignored `data/song-download-sources.local.json`; existing resolved URLs are preserved and removed candidate rows are dropped. Both manifest generation and automatic source resolution put active billion-stream candidates first in source-rank order, then retain candidate-file order for the remaining founder and crossover picks. `npm run resolve:youtube -- 10` fills a reviewable batch, while larger limits continue through that priority queue. Selection prefers verified, artist, official, VEVO, and Topic channels; rejects live, karaoke, remix, sped/slowed, cover, edit, snippet, demo, compilation, and implausibly short or long results; and favors results containing every credited artist. `npm run audit:sources` independently checks completeness, aliases, title/artist agreement, durations, altered-version labels, duplicate video IDs, collaborator coverage, remaster warnings, and provenance before downloads.
   ...
   A fingerprint match is not sufficient when the song prominently samples another recording. The previous `death bed (coffee for your head)` source was an unrelated-channel “HQ Audio” upload that passed a short comparison dominated by the beabadoobee sample. User playback feedback exposed it. Sample-heavy songs now require official artist/label/Topic provenance in addition to the fingerprint; the corrected Death Bed source is the Columbia-provided Powfu upload and credits beabadoobee.
...
After a real intro-identification review, difficulty weights intro identification at 45%, stream/broad reach at 35%, Gen-Z/current relevance at 15%, and longevity at 5%. Before that review, waveform audibility is only a 10% proxy because loudness cannot measure whether a listener recognizes the sound; reach and audience familiarity carry most of the provisional result. See [CATALOG.md](CATALOG.md) for both formulas and fixed thresholds.
